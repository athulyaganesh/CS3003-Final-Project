...
public class StaticTypeCheck {
        ...
        if (e instanceof Binary) {
            ...
            if (b.op.ArithmeticOp( ))
                if (typeOf(b.term1,tm)== Type.FLOAT || typeOf(b.term2,tm)== Type.FLOAT)
                    return (Type.FLOAT);
                else if (typeOf(b.term1,tm)== Type.DOUBLE || typeOf(b.term2,tm)== Type.DOUBLE) //MODIFIED
                    return (Type.DOUBLE); 
	...
	if (e instanceof Binary) {
        ...
        if (typ1 == Type.INT && typ2 == Type.DOUBLE)  //MODIEFIED 
			check( true, "should never reach here");
	...
	if (e instanceof Unary) {
		...
		check( typ == Type.INT || typ == Type.FLOAT || typ == Type.DOUBLE, "type error for " + u.op); //modified 
	    else if (u.op.intOp( ) || u.op.floatOp( ) || u.op.charOp( ) || u.op.doubleOp( )) //modified 
		...
		for (int i=0; i<c.args.size(); i++) {
			...
			if (param_types.get(i).equals(Type.FLOAT)) {
				check((current_arg_type == Type.FLOAT) ||
					(current_arg_type  == Type.INT || current_arg_type == Type.DOUBLE), 
				"argument passed to function " + c.name + "not coercible to type float");	// MODIFIED
			}
			else {
