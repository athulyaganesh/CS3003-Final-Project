...
public class Parser {
	...
	if (token.type().equals(TokenType.Int)) {
		...
		t = Type.VOID;
	} else if(token.type().equals(TokenType.Double)){ //MODIFIED 
        t = Type.DOUBLE; 
	...
	if (token.type().equals(TokenType.LeftBrace)) {
		...
		s = ifStatement();
    } else if(token.type().equals(TokenType.For)){ //MODIFIED//
      s = forStatement();
    ...
    private Loop forStatement(){ //MODIFIED//
      match(token.type());
      match(TokenType.LeftParen);
      Expression test = expression();
      match(TokenType.RightParen);
    ...
    private Expression modulo() { //MODIFIED
        Expression e = term();
        while (isModuloOp()){
            Operator op= new Operator(match(token.type()));
            Expression term2= term();
	...
	} else if(token.type().equals(TokenType.DoubleLiteral)){ //MODIFIED//
    ...
    private boolean isModuloOp(){ //MODIFIED//
...
//        Parser parser  = new Parser(new Lexer("hello.cpp"));
        Parser parser  = new Parser(new Lexer("Main.java"));//was Test.java 
