...
public class CodeGen {
    ...
    void M (Assignment a, SymbolTable symtable, JasminFile jfile) throws IOException {
		...
		if (symtable.containsKey(target)) {
			...
			if (target_type.equals(Type.INT) || target_type.equals(Type.CHAR) || target_type.equals(Type.BOOL)) {
				...
				store = "fstore";
			} else if (target_type.equals(Type.DOUBLE)){ //MODIFIED//
        store = "dstore";
      } else {
				throw new IllegalArgumentException("should never reach here");
			}
		jfile.writeln(store + " " + symtable.getIndex((Variable) a.target));
			...
			descriptor.equals(Type.CHAR))
				type = "I";
      else if (descriptor.equals(Type.DOUBLE)) //MODIFIED//
        type = "D";
			else // it's a float
				type = "F";
	...
	if (e_type.equals(Type.FLOAT)) 
		...
		print_type = "C";
  else if (e_type.equals(Type.DOUBLE)) //MODIFIED//
    print_type = "D";
	else //It's a Bool
		print_type = "Z";
	...
	else if (j_type.equals("D")) //MODIFIED//
    ...
    private Type typeOf(Expression e, SymbolTable sym) {
        ...
        if (e instanceof Binary) {
            ...
            if (b.op.ArithmeticOp( ))
                if (typeOf(b.term1,sym)== Type.FLOAT)
                    return (Type.FLOAT);
                else if (typeOf(b.term1,sym) == Type.DOUBLE) //MODIFED//
                    return (Type.DOUBLE);
	    ...
	    System.out.println("looking in Unary");
            ...
            else if (u.op.doubleOp()) return (Type.DOUBLE); //MODIFIED//
    ...
    void applyBinary (Operator op, JasminFile jfile) throws IOException {
      ...
      return;
        ...
        } if (op.val.equals(Operator.INT_MODULO)){ //MODIFIED//
      jfile.writeln("imod");
      return; 
		}
        // student exercise
	...
	*/
	...
	if (op.val.equals(Operator.FLOAT_LT)) {
		...
		return;
			...
			jfile.writeln("fdiv");
            return; 
		} if (op.val.equals(Operator.FLOAT_MODULO)) {
			jfile.writeln("fmod");
            return; 
		}
    //MODIFIED
    if (op.val.equals(Operator.DOUBLE_LT)) {
		...
		return;
			...
			jfile.writeln("ddiv");
            return; 
		} if (op.val.equals(Operator.DOUBLE_MODULO)) {
			jfile.writeln("dmod");
            return; 
		}
    ...
    void M (Expression e, SymbolTable symtable, JasminFile jfile) throws IOException { 
        if (e instanceof Value) {
			if (e instanceof IntValue || e instanceof FloatValue || e instanceof DoubleValue) {
    	...
    	return;
			...
			// I think applyBinary should handle the work here (I don't know if this is good design?)
            Binary b = (Binary)e;
